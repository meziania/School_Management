...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabase.from('classes').select('*').eq('is_active', true).order('name')
...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await supabase.from('classes').insert({ ...parsed.data, school_id }).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ data, error: null })
  } catch { return NextResponse.json({ error: 'Erreur interne.' }, { status: 500 }) }
}
